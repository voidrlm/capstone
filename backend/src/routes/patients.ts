        ...
        data: {
          ...
          debug: includeDebug
            ? {
                ...
                extractedVaccinationCount: parsedVaccinations.length,
              }
            : undefined,
        },
      });
...
// ---------------------------------------------------------------------------
// POST /api/patients/:id/lab-results – manually add lab result
// ---------------------------------------------------------------------------
router.post(
  "/:id/lab-results",
  authMiddleware,
  async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      await query(
        `INSERT INTO lab_results (patient_id, test_name, result, date, reference_range)
         VALUES ($1, $2, $3, $4, $5)
...
// ---------------------------------------------------------------------------
// POST /api/patients/:id/visits – manually add visit
// ---------------------------------------------------------------------------
router.post(
  "/:id/visits",
  authMiddleware,
  async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      if (!reason || !visit_date) {
        res.status(400).json({ success: false, error: { message: "reason and visit_date are required" } });
        return;
      }
      ...
      await query(
        ...
        [id, parsedDate.toISOString(), reason, doctor_name || null, doctor_specialty || null],
      );
...
// ---------------------------------------------------------------------------
// POST /api/patients/:id/vaccinations – manually add vaccination
// ---------------------------------------------------------------------------
router.post(
  "/:id/vaccinations",
  authMiddleware,
...
// ---------------------------------------------------------------------------
// POST /api/patients/:id/diagnoses – manually add diagnosis
// ---------------------------------------------------------------------------
router.post(
  "/:id/diagnoses",
  authMiddleware,
